...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
				...
				END sls_due_dt, -- Recalculate sales if original value is missing or incorrect
				CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales !=  sls_quantity * ABS(sls_price) THEN sls_quantity * ABS(sls_price)
					ELSE sls_sales
			   END AS sls_sales, -- Derive price if original value in invaild
			   sls_quantity, 
		...
		INSERT INTO silver.erp_cust_az12 (
				...
				gen)
		SELECT 
				CASE WHEN UPPER(cid) LIKE 'NAS%' THEN SUBSTRING(cid,4,len(cid))  -- Remove 'NAS' prefix if present
					ELSE cid
				END AS cid,
		...
		SELECT REPLACE(cid,'-','') as cid,
			   CASE WHEN NULLIF(TRIM(cntry),'') IS NULL THEN 'N/A'
